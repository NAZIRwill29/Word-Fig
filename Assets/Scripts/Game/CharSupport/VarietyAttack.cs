...
public class VarietyAttack : Shooting
{
    ...
    public bool isMoveAttackSwing, isTeleportOnce, isTeleportRandom, isTeleportWithSwing, isTeleportChase;
    [Tooltip("for teleport position")]
    [SerializeField]
    private float teleportMinX, teleportMaxX, teleportMinY, teleportMaxY, moveAttackSpeedX = 3, moveAttackSpeedY = 3;
    ...
    {
        if (isMoveAttackSwing)
            AttackTypeSwing();
        if (Time.time - lastMove > cooldownTrigger)
        {
            lastMove = Time.time;
            // anim.SetTrigger("Move");
            ChangeChaseSpeed(moveAttackSpeedX, moveAttackSpeedY);
